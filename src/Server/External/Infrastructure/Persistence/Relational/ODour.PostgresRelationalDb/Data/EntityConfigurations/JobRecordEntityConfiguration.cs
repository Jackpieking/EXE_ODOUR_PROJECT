...
internal sealed class JobRecordEntityConfiguration : IEntityTypeConfiguration<JobRecordEntity>
{
    public void Configure(EntityTypeBuilder<JobRecordEntity> builder)
    {
        ...
        builder
            .Property(propertyExpression: userDetail => userDetail.CancelledOn)
            .HasColumnType(typeName: DatabaseNativeType.TIMESTAMPTZ)
            .IsRequired(required: true);

        builder
            .Property(propertyExpression: userDetail => userDetail.FailureCount)
            .IsRequired(required: true);

        builder
            .Property(propertyExpression: userDetail => userDetail.FailureReason)
            .HasColumnType(typeName: DatabaseNativeType.JSONB)
            .IsRequired(required: true);
    }
}
