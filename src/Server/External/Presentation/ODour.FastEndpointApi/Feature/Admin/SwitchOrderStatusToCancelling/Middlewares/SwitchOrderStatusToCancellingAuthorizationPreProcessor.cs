...
internal sealed class SwitchOrderStatusToCancellingAuthorizationPreProcessor
    : PreProcessor<SwitchOrderStatusToCancellingRequest, SwitchOrderStatusToCancellingStateBag>
{
    ...
    public SwitchOrderStatusToCancellingAuthorizationPreProcessor(
    ...
    )
    {
        ...
        if (!validateTokenResult.IsValid || tokenExpireTime <= DateTime.UtcNow)
        {
            await SendResponseAsync(
                statusCode: SwitchOrderStatusToCancellingResponseStatusCode.UN_AUTHORIZED,
        ...
        var cacheKey =
            $"{nameof(SwitchOrderStatusToCancellingRequest)}__AUTHORIZATION_CHECK__{accessTokenId}";
        ...
        if (Equals(objA: foundAuthorizedAccessToken, objB: AppCacheModel<bool>.NotFound))
        {
            ...
            var isRefreshTokenFound =
                await unitOfWork.Value.SwitchOrderStatusToCancellingRepository.IsRefreshTokenFoundQueryAsync(
            ...
            {
                await SendResponseAsync(
                    statusCode: SwitchOrderStatusToCancellingResponseStatusCode.FORBIDDEN,
                    appRequest: context.Request,
                    context: context.HttpContext,
                    ct: ct
                );
    ...
    private static Task SendResponseAsync(
        SwitchOrderStatusToCancellingResponseStatusCode statusCode,
        SwitchOrderStatusToCancellingRequest appRequest,
        HttpContext context,
        CancellationToken ct
    )
    {
