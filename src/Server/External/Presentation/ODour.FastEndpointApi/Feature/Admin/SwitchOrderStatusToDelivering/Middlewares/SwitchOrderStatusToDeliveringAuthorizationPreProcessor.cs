...
internal sealed class SwitchOrderStatusToDeliveringAuthorizationPreProcessor
    : PreProcessor<SwitchOrderStatusToDeliveringRequest, SwitchOrderStatusToDeliveringStateBag>
{
    ...
    public SwitchOrderStatusToDeliveringAuthorizationPreProcessor(
    ...
    )
    {
        ...
        if (!validateTokenResult.IsValid || tokenExpireTime <= DateTime.UtcNow)
        {
            await SendResponseAsync(
                statusCode: SwitchOrderStatusToDeliveringResponseStatusCode.UN_AUTHORIZED,
        ...
        var cacheKey =
            $"{nameof(SwitchOrderStatusToDeliveringRequest)}__AUTHORIZATION_CHECK__{accessTokenId}";
        ...
        if (Equals(objA: foundAuthorizedAccessToken, objB: AppCacheModel<bool>.NotFound))
        {
            ...
            var isRefreshTokenFound =
                await unitOfWork.Value.SwitchOrderStatusToDeliveringRepository.IsRefreshTokenFoundQueryAsync(
            ...
            {
                await SendResponseAsync(
                    statusCode: SwitchOrderStatusToDeliveringResponseStatusCode.FORBIDDEN,
                    appRequest: context.Request,
                    context: context.HttpContext,
                    ct: ct
                );
    ...
    private static Task SendResponseAsync(
        SwitchOrderStatusToDeliveringResponseStatusCode statusCode,
        SwitchOrderStatusToDeliveringRequest appRequest,
        HttpContext context,
        CancellationToken ct
    )
    {
