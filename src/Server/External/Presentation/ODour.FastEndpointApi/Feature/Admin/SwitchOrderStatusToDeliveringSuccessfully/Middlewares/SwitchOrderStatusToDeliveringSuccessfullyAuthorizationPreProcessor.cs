...
internal sealed class SwitchOrderStatusToDeliveringSuccessfullyAuthorizationPreProcessor
    : PreProcessor<
    ...
    >
{
    ...
    public SwitchOrderStatusToDeliveringSuccessfullyAuthorizationPreProcessor(
    ...
    )
    {
        ...
        if (!validateTokenResult.IsValid || tokenExpireTime <= DateTime.UtcNow)
        {
            await SendResponseAsync(
                statusCode: SwitchOrderStatusToDeliveringSuccessfullyResponseStatusCode.UN_AUTHORIZED,
        ...
        var cacheKey =
            $"{nameof(SwitchOrderStatusToDeliveringSuccessfullyRequest)}__AUTHORIZATION_CHECK__{accessTokenId}";
        ...
        if (Equals(objA: foundAuthorizedAccessToken, objB: AppCacheModel<bool>.NotFound))
        {
            ...
            var isRefreshTokenFound =
                await unitOfWork.Value.SwitchOrderStatusToDeliveringSuccessfullyRepository.IsRefreshTokenFoundQueryAsync(
            ...
            {
                await SendResponseAsync(
                    statusCode: SwitchOrderStatusToDeliveringSuccessfullyResponseStatusCode.FORBIDDEN,
                    appRequest: context.Request,
                    context: context.HttpContext,
                    ct: ct
                );
    ...
    private static Task SendResponseAsync(
        SwitchOrderStatusToDeliveringSuccessfullyResponseStatusCode statusCode,
        SwitchOrderStatusToDeliveringSuccessfullyRequest appRequest,
        HttpContext context,
        CancellationToken ct
    )
    {
