    ...
    : PreProcessor<EmptyRequest, GetCartDetailStateBag>
{
    ...
    )
    {
        ...
        var cacheKey = $"{nameof(GetCartDetailRequest)}__AUTHORIZATION_CHECK__{accessTokenId}";
        ...
        if (Equals(objA: foundAuthorizedAccessToken, objB: AppCacheModel<bool>.NotFound))
        {
            ...
            var isRefreshTokenFound =
                await unitOfWork.Value.GetCartDetailRepository.IsRefreshTokenFoundQueryAsync(
