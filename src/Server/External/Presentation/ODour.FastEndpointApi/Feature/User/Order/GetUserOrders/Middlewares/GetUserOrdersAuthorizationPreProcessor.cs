...
internal sealed class GetUserOrdersAuthorizationPreProcessor
    : PreProcessor<GetUserOrdersRequest, GetUserOrdersStateBag>
{
    ...
    public GetUserOrdersAuthorizationPreProcessor(
    ...
    )
    {
        ...
        )
        {
            await SendResponseAsync(
                statusCode: GetUserOrdersResponseStatusCode.UN_AUTHORIZED,
        ...
        #region Part1
        // Get refresh token.
        var refreshToken = await unitOfWork.Value.GetUserOrdersRepository.GetRefreshTokenQueryAsync(
        ...
        {
            await SendResponseAsync(
                statusCode: GetUserOrdersResponseStatusCode.FORBIDDEN,
                appRequest: context.Request,
                context: context.HttpContext,
                ct: ct
            );
        ...
        var isUserTemporarilyRemoved =
            await unitOfWork.Value.GetUserOrdersRepository.IsUserBannedQueryAsync(
                userId: foundUser.Id,
                ct: ct
            );
    ...
    private static Task SendResponseAsync(
        GetUserOrdersResponseStatusCode statusCode,
        GetUserOrdersRequest appRequest,
        HttpContext context,
        CancellationToken ct
    )
    {
